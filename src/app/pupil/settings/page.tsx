    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        <div className="relative w-16 h-16 mx-auto mb-4">
          <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 animate-pulse" />
...
export default function PupilSettingsPage() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('id, full_name, display_name, email, phone, bio, class, class_arm, role')
        .eq('id', user.id)
        .single()
      ...
      if (data) {
        ...
        try {
          const savedNotifications = localStorage.getItem('pupil_notifications')
          if (savedNotifications) {
            setNotifications(JSON.parse(savedNotifications))
          }

          const savedPreferences = localStorage.getItem('pupil_preferences')
          if (savedPreferences) {
  ...
  const handleNotificationsSave = () => {
    try {
      localStorage.setItem('pupil_notifications', JSON.stringify(notifications))
  ...
  const handlePreferencesSave = () => {
    try {
      localStorage.setItem('pupil_preferences', JSON.stringify(preferences))
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      localStorage.removeItem('pupil_notifications')
      localStorage.removeItem('pupil_preferences')
      await logout()
  ...
  const displayName = profile?.display_name || profile?.full_name || user?.full_name || 'Pupil'
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="text-center text-xs text-slate-400 pt-4 border-t border-slate-200/50">
          <p>Vincollins Schools Pupil Settings • Geared Towards Excellence</p>
        ...
        <AlertDialogContent className="rounded-2xl max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-500">
              ...
              All your data, including assignments, notes, scores, and profile information, 
